...
/**
 * Patient Chat Page
 * Loads conversation between logged-in patient and a doctor (by doctor_id or appointment_id)
 */
...
$sender_account = 'patient_' . $patient_id;
...
try {
    ...
    if (!empty($_GET['doctor_id'])) {
        ...
        $stmt = $conn->prepare("SELECT doctor_id FROM appointments WHERE id = ? LIMIT 1");
    ...
    if (!$doctor_id) {
        throw new Exception('Missing doctor_id or appointment context.');
    }

    $receiver_account = 'doctor_' . $doctor_id;

    // Fetch doctor info
    $doctor = ['name' => 'Doctor', 'profile_image' => 'assets/img/doctors/doctor-01.jpg'];
    $stmt = $conn->prepare("SELECT d.name AS name, dp.profile_image AS profile_image FROM doctors d LEFT JOIN doctor_profiles dp ON d.id = dp.doctor_id WHERE d.id = ? LIMIT 1");
    $stmt->bind_param('i', $doctor_id);
    $stmt->execute();
    $r = $stmt->get_result();
    if ($r && $r->num_rows) {
        $doctor = array_merge($doctor, $r->fetch_assoc());
    }
    $stmt->close();
    ...
    error_log('Chat (patient) error: ' . $e->getMessage());
...
<html lang="en">
	<head>
		<meta charset="utf-8">
		<title>TeleRx - Patient Chat</title>
...
<body class="main-chat-blk">
												...
												<li class="user-list-item active">
													<a href="javascript:void(0);">
														<div class="avatar avatar-online">
															<img src="<?php echo htmlspecialchars($doctor['profile_image']); ?>" class="rounded-circle" alt="image">
														</div>
										...
										<div class="chat-header">
											<div class="user-details">
												...
												<figure class="avatar avatar-online">
													<img src="<?php echo htmlspecialchars($doctor['profile_image']); ?>" alt="image">
												</figure>
												<div class="mt-1">
													<h5><?php echo htmlspecialchars($doctor['name']); ?></h5>
										...
										<div class="messages" id="messages">
											...
											<div class="chats <?php echo $is_sent ? 'chats-right' : ''; ?>">
												...
												<div class="chat-avatar">
													<img src="<?php echo htmlspecialchars($doctor['profile_image']); ?>" class="dreams_chat" alt="image">
												</div>
												<?php endif; ?>
												<div class="chat-content">
													<div class="chat-profile-name <?php echo $is_sent ? 'text-end justify-content-end' : ''; ?>">
														<h6><?php echo $is_sent ? 'You' : htmlspecialchars($doctor['name']); ?><span><?php echo $created; ?></span></h6>
													</div>
													<div class="message-content"><?php echo $msg_html; ?></div>
												</div>
												<?php if ($is_sent): ?>
												<div class="chat-avatar">
													<img src="<?php echo htmlspecialchars($_SESSION['profile_image'] ?? 'assets/img/patients/patient.jpg'); ?>" class="dreams_chat" alt="image">
												</div>
												<?php endif; ?>
											</div>
											<?php endforeach; ?>
    ...
    (function(){
        ...
        var doctorName = <?php echo json_encode($doctor['name']); ?>;
        var peerImg = <?php echo json_encode($doctor['profile_image']); ?>;
        var myImg = <?php echo json_encode($_SESSION['profile_image'] ?? 'assets/img/patients/patient.jpg'); ?>;
        ...
        function appendMessage(msgHtml, sent, time) {
            ...
            $profile.append($('<h6>').text(sent ? 'You' : doctorName).append($('<span>').text(time)));
...
// Get all doctors the patient has appointments with
$conn = getDBConnection();
$stmt = $conn->prepare("
    SELECT DISTINCT d.id, d.name, dp.profile_image 
    FROM doctors d 
...
$target_doctor_id = isset($_GET['doctor_id']) ? (int)$_GET['doctor_id'] : (count($doctors) > 0 ? $doctors[0]['id'] : 0);
$target_doctor_name = "Select a Doctor";
$target_doctor_img = "assets/img/doctors-dashboard/doctor-profile-img.jpg";

foreach ($doctors as $d) {
    if ($d['id'] == $target_doctor_id) {
        $target_doctor_name = $d['name'];
        if (!empty($d['profile_image'])) $target_doctor_img = $d['profile_image'];
        break;
    }
}
?>
...
<html lang="en">
	...
	<body class="main-chat-blk">
											...
											<div class="megamenu-wrapper">
												<div class="row">
													...
													<div class="col-lg-2">
														<div class="single-demo ">
															<div class="demo-img">
																<a href="index-4.html" class="inner-demo-img"><img src="assets/img/home/home-01.jpg" class="img-fluid " alt="img"></a>
															</div>
															<div class="demo-info">
									...
									<li class="has-submenu active">
										...
										<ul class="submenu">
											...
											<li class="has-submenu">
												...
												<ul class="submenu inner-submenu">
													<li><a href="search.php">Search Doctor 1</a></li>
													<li><a href="doctors.php">Search Doctor 2</a></li>
												</ul>
											</li>
											<li class="has-submenu">
												<a href="javascript:void(0);">Doctor Profile</a>
												<ul class="submenu inner-submenu">
													<li><a href="doctor-profile.php">Doctor Profile 1</a></li>
							...
							<ul class="nav header-navbar-rht">
								...
								<li class="nav-item dropdown has-arrow logged-item">
									<a href="#" class="nav-link ps-0" data-bs-toggle="dropdown">
										<span class="user-img">
											<img class="rounded-circle" src="assets/img/doctors-dashboard/profile-06.jpg" width="31" alt="Darren Elder">
										</span>
									</a>
									<div class="dropdown-menu dropdown-menu-end">
									   ...
									   <!-- <div class="left-chat-title all-chats">
											...
											<div class="swiper-container">
												<div class="swiper-wrapper">
													...
													<div class="swiper-slide">
														<div class="top-contacts-box">
															<div class="profile-img online">
																<img src="assets/img/doctors-dashboard/profile-08.jpg" alt="Img">
															</div>
														</div>
													</div>
													<div class="swiper-slide">
														<div class="top-contacts-box">
															<div class="profile-img online">
																<img src="assets/img/doctors-dashboard/profile-06.jpg" alt="Img">
															</div>
														</div>
													</div>
													<div class="swiper-slide">
														<div class="top-contacts-box">
															<div class="profile-img online">
																<img src="assets/img/doctors-dashboard/profile-07.jpg" alt="Img">
															</div>
														</div>
													</div>
												</div>
										...
										<!-- <div class="sidebar-body chat-body" id="chatsidebar">
											...
											<ul class="user-list">
												...
												<li class="user-list-item" data-doctor-id="<?php echo $d['id']; ?>">
													<a href="chat.php?doctor_id=<?php echo $d['id']; ?>" >
														<div class="avatar avatar-online">
															<img src="<?php echo !empty($d['profile_image']) ? htmlspecialchars($d['profile_image']) : 'assets/img/doctors-dashboard/doctor-profile-img.jpg'; ?>" alt="image">
														</div>
														<div class="users-list-body">
															<div>
							...
							<!-- <div class="chat chat-messages" id="middle">
								...
								<div class="chat-footer">
									<form id="chat-form">
										...
										<input type="text" id="chat-message-input" class="form-control chat_form" placeholder="Type your message here...">
										<div class="form-buttons">
											<button class="btn send-btn" type="submit">
												<i class="isax isax-send-25"></i>
											</button>
		...
		<!-- Agora Chat SDK -->
                <script src="https://download.agora.io/sdk/release/agora-rtm-sdk-1.5.0.js"></script>
                <!-- Custom Chat JS -->
                <script src="assets/js/agora-chat.js"></script>
